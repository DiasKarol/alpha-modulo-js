...
function exponential(num1, num2) {
    try {
        if (isNaN(num1) || isNaN(num2)) throw 'exponential'
    ...
    if (num2 == 1) {
        ...
        return multiply(num1, exponential(num1, subtract(num2, 1)))
    }
}

function division(num1, num2) {
    try {
        if (isNaN(num1) || isNaN(num2)) throw `Impossible to divide ${num1} / ${num2}`
    ...
    return Number(num1) >= Number(num2) ? sum(division(subtract(num1, num2), num2), 1) : 0
}
